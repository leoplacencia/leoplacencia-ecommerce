        ...
        <table class="table table-striped table-hover table-bordered">
            ...
            <tbody>
                @foreach($cart as $item)
                    <tr>
                        <td><img width="50px" class="img-responsive img-rounded" src="{{ $item->image }}" alt=""></td>
                        <td>{{ $item->name }}</td>
                        <td>{{ number_format($item->price, 2) }}</td>
                        <td>
                            <input type="number" min="1" max="100" value="{{ $item->quantity }}" id="product_{{ $item->id }}">
                            <button class="btn btn-warning btn-update-item" data-href="{{ $item->slug}}" data-id="{{ $item->id }}">
